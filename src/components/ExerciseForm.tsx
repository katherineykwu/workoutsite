...
export default function ExerciseForm({ exercise, routineId, onSave, onCancel }: ExerciseFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    onSave({ id: exerciseId, name, sets, reps, restSeconds, notes, videoType, videoUrl });
  }

  const inputClass = "w-full px-4 py-3 bg-white/10 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-[#FF1A66] focus:border-transparent transition-all";

  return (
    <form onSubmit={handleSubmit} className="bg-white/5 rounded-2xl border border-white/10 p-6 space-y-5">
      <h3 className="font-bold text-white text-lg">
        {exercise ? "Edit Exercise" : "New Exercise"}
      </h3>

      <div>
        <label className="block text-sm font-semibold text-white/60 mb-1.5">Exercise Name</label>
      ...
      {videoType === "upload" && (
        <div>
          <input type="file" accept="video/*" onChange={handleFileUpload} disabled={uploading}
            className="w-full text-sm text-white/50 file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-[#FF1A66] file:text-white hover:file:opacity-80 file:transition-opacity file:cursor-pointer" />
          {uploadProgress && <p className={`text-sm mt-2 ${videoUrl ? "text-[#FF1A66]" : "text-white/40"}`}>{uploadProgress}</p>}
        </div>
      )}
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="button" onClick={onCancel}
          className="bg-white/10 border border-white/10 text-white/60 px-6 py-3 rounded-xl font-semibold hover:bg-white/15 transition-colors">
          Cancel
        </button>
      </div>
    </form>
